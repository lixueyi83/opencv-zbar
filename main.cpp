...
int main_webcam(int argc, char **argv);

/*
*   runtime measurement utility function
*/
...
{
    main_image(argc, argv);
}
/****************************************************************************************
*   read an image and feed into zbar to decode
*/
int main_image(int argc, char **argv) 
{    
    ...
    */
    ...
    while(true)
    {
        ...
        //frame = imread("../images/2.jpg", CV_LOAD_IMAGE_COLOR);
        
        String images = "../images/*.jpg";
        vector<String> image_buf;
        glob(images, image_buf);
        
        for(int i=0; i<image_buf.size(); i++)
        {
            frame = imread(image_buf[i], CV_LOAD_IMAGE_COLOR);
            ...
            */
            //scanner.scan(image);
            RUNTIME_MEAS(scanner.scan, image);
